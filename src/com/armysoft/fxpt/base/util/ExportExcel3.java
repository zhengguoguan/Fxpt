...
public class ExportExcel3 { 
	...
	public ExportExcel3(String excelName, String sheetTitle,List exportlist)
	  {
...
//			WritableSheet.setRowView( int i, int height); // 作用是指定第i+1行的高度，比如：
			...
			for (int j=0 ;j<columnCount;j++){
			    wsheet.addCell(new Label(j,0, columnCaption[j].toString(),wcfFQ));   //列头部标�?
			}
			//行内�?
			Object rowData[];
			...
			for(int i=0;i<list.size();i++){   
				...
				for (int j=0 ;j<columnCount;j++){
				    ...
				    wsheet.addCell(new Label(j, i+1, value.toString(),wcfFQ));   //姓名
				}
			}   		
