...
{     
	...
	/**     * 导入网络图片到缓冲区     */    
	     ...
	     public BufferedImage modifyImage(BufferedImage img, Object content, int x,            int y, int size) 
	     {        
	    try {            
	     int w = img.getWidth();            
	     int h = img.getHeight();          
	     ...
	     Font newfont = new Font("microsoft yahei", Font.PLAIN, size);
	     if (newfont != null)     
	    	g.setFont(newfont);           
	     // 验证输出位置的纵坐标和横坐标        
	     if (x >= h || y >= w) {             
	     ...
	     /**     * 修改图片,返回修改后的图片缓冲区（只输出一行文本）     *      * 时间:2007-10-8     *      * @param img     * @return     */  
	     ...
	     public BufferedImage modifyImagetogeter(BufferedImage b, BufferedImage d) {  
	    	 try {          
	    	 int w = Integer.valueOf((int) (b.getWidth()/2.1));     
	    	 int h = Integer.valueOf((int) (b.getHeight()/2.1));            
	    	 g = d.createGraphics();         
	    	 g.drawImage(b, 70, 478, w, h, null);          
	    	 g.dispose();       
	    	 } catch (Exception e) {       
	    		 System.out.println(e.getMessage());   
	    	 }      
	      ...
	      public static void main(String[] args) {     
	    		...
	    		// BufferedImage b = tt .loadImageLocal("E:\\文件(word,excel,pdf,ppt.txt)\\zte-logo.png");       
	    		 tt.writeImageLocal("D:\\cc.jpg",tt.modifyImage(d,"曹原",200,200,30)); 
	    				 //往图片上写文件            
