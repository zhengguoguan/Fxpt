...
public class ExportExcel { 
	...
	public ExportExcel(String excelName, String sheetTitle,List exportlist)
	  {
...
//		List columnFieldList=new ArrayList();
		List<String> columnCaptionList=new ArrayList();
...
//			columnFieldList.add(erowData[0].toString());
			columnCaptionList.add(erowData[0].toString());
		}
//		columnField= columnFieldList.toArray();
	...
	public  boolean exportExcel_Applicant(HttpServletRequest request,HttpServletResponse response,List list ) {		
		try{ 
			int columnCount=columnCaption.length;
			...
			for(int i=0;i<list.size();i++){   
				...
				for (int j=0 ;j<columnCount;j++){
					value.delete(0, value.length());
					   ...
					   value.append(rowData[j].toString());
									  }
                    }
