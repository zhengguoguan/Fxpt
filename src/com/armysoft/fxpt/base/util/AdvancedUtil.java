...
public class AdvancedUtil {
	 ...
	 * @param savePath 保存路径
	 */
	public static String getQRCode(String ticket, String savePath,HttpServletRequest request) {
		...
		try {
			...
			File newFile = new File(request.getSession().getServletContext().getRealPath(filePath));
			if(!newFile.getParentFile().exists()){
				newFile.getParentFile().mkdirs();
			}
			// 将微信服务器返回的输入流写入文件
			BufferedInputStream bis = new BufferedInputStream(conn.getInputStream());
			FileOutputStream fos = new FileOutputStream(new File(request.getSession().getServletContext().getRealPath(filePath)));
			...
			log.info("根据ticket换取二维码成功，filePath=" + request.getSession().getServletContext().getRealPath(filePath));
		} catch (Exception e) {
			filePath = null;
	...
	public static WeixinMedia uploadMedia(String accessToken, String type, String mediaFileUrl) {
		...
		try {
			...
			System.out.println(mediaFileUrl);
	...
	public static void main(String args[],HttpServletRequest request, HttpServletResponse response) {
		...
		// 根据ticket换取二维码
		getQRCode(ticket, savePath, request);
