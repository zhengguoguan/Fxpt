...
// [版 本 号] ver4.0.1
//====================================================================================================
var  submmitDivId="submitTip";        //提交时验证的冒泡提示的ID
var redTip="<font color=red>&nbsp;&nbsp;&nbsp;&nbsp;必填*</font>";
(function($) {
...
$.formValidator = 
{
	...
	//提交后，定位漂浮层
	localTooltipAfterSummit : function(element)
	{
		//left
...
　　　　}
		...
		　　　　　　current = current.offsetParent;
　　　　}
		$("#"+submmitDivId).css({"top":(actualTop+20)+"px","left":(actualLeft+70)+"px"});
	},
...
/***不检查某元素
 * obj     不检查的元素对象
 * sourceStr  元素对象在页面显示的初始value值
 * desStr     元素对象在页面显示的最后value值
 * id         该元素的ID
 */
function uncheck(obj,sourceStr,desStr,id)
{
	if(obj.value==sourceStr)
	{
		$(id).attr("disabled",true).unFormValidator(true);
		obj.value = desStr;
	}
	else
	{
		$(id).attr("disabled",false).unFormValidator(false);
		obj.value = sourceStr;
	}
}
 ...
 * 增加提交验证冒泡提示功能
 * @param formID  表单ID
 * @param disappearSeconds     隔几秒后提示框自动消失
 * @return
 */
function formInitConfig(formID,disappearSeconds){
	
	var submitTipMsg="<div id='"+submmitDivId+"' style='position:absolute;z-index:56002'><span class='fv_tooltip' style='display: block; opacity: 0.95;'><span class='top' id='fv_content_submit' style='display:block'></span><b class='bottom' style='display:block'></b></span></div>"
		$("body").append(submitTipMsg);
		$("#"+submmitDivId).hide();
		var stopTip;
		var toolTip;
	$.formValidator.initConfig({formID:formID,autoTip:true,tidyMode:true,onError:function(msg,obj,errorlist){
			$("#"+submmitDivId).empty();
			toolTip="<span class='fv_tooltip' style='display: block; opacity: 0.95;'><span class='top' id='fv_content_submit' style='display:block'>"+msg+"</span><b class='bottom' style='display:block'></b></span>"
			$("#"+submmitDivId).append(toolTip);
			$("#"+submmitDivId).show();
$.formValidator.localTooltipAfterSummit(obj);
 stopTip =setTimeout(function(){$("#"+submmitDivId).hide();
 clearTimeout(stopTip);
},disappearSeconds)
	}});
}
/** 按比较的类型验证
 ...
 * @param idStr  jquery选择器表达式
 * @param onShowStr 初始显示时的提示信息
 * @param onFocusStr 获得焦点时的提示信息
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param min          最小长度/值
 * @param max          最大长度/值
 * @param type         比较类型,包括如下：
 (对select无效)
"size":表示比较字符长度 ，默认值
"number":数值型比较
"string":字符型比较
...
function validateMinMax(isEmpty,idStr,onShow,onFocus,onCorrect,onError,min,max,type){
	if (isEmpty==false){
		$(idStr).after(redTip);
	}

	$(idStr).formValidator({onShow:onShow,onFocus:onFocus,
		onCorrect:onCorrect}).inputValidator({min:min,max:max,type:type,onError:onError});
}
 ...
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param desID        要跟源目标进行比较的目标ID
 * @param comparedError  比较后发生错误的提示
 * @return
 */
function validateRepassword(isEmpty,idStr,onShow,onFocus,onCorrect,onError,desID,comparedError){
	if (isEmpty==false){
		$(idStr).after(redTip);
	}
	$(idStr).formValidator({onShow:onShow,onFocus:onFocus,onCorrect:onCorrect}).inputValidator({min:1,
		onError:onError}).compareValidator({desID:desID,operateor:"=",onError:comparedError});
}
/**验证单选下拉列表
 * 
 * @param isEmpty  是否允许为空
 ...
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param defaultValue  所有input和select表单。如果你不设置就保持原值，一旦设置就设为默认值。
 * @return
 */
function validateSelection(isEmpty,idStr,onShow,onFocus,onCorrect,onError,defaultValue){
	if (isEmpty==false){
		$(idStr).after(redTip);
	}
	$(idStr).formValidator({onShow:onShow,onFocus:onFocus,onCorrect:onCorrect,
		defaultValue:defaultValue}).inputValidator({min:1,onError: onError});
}
/**验证复选框与多选select控件
 * 
 * @param isEmpty  是否允许为空
 ...
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param defaultValue  数组形式的字符串，如 ["7","8"]
 * @param min           最少选中的个数
 * @param max           最多选中的个数
 * @return
 */
function validateCheckbox_Multipleselect(isEmpty,idStr,onShow,onFocus,onCorrect,onError,defaultValue,min,max){
	if (isEmpty==false){
		//$(idStr).after(redTip);
	}
	$(idStr).formValidator({onShow:onShow,onFocus:onFocus,onCorrect:onCorrect,
		defaultValue:defaultValue}).inputValidator({min:min,max:max,onError:onError});
}
/**验证单选按钮
 * 
 * @param idStr  jquery选择器表达式
 ...
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param defaultValue  数组形式的字符串，如 ["4"]
 * @return
 */
function validateRadio(idStr,onShow,onFocus,onCorrect,onError,defaultValue){
	
	$(idStr).formValidator({onShow:onShow,onFocus:onFocus,onCorrect:onCorrect,
		defaultValue:defaultValue}).inputValidator({min:1,max:1,onError:onError});
}
/**按规则验证
 * 
 * @param isEmpty  是否允许为空
 ...
 * @param onCorrectStr 输入正确时的提示信息
 * @param onError      输入正确错误时的提示信息
 * @param isEnum       是否调用枚举,目前支持的枚举如下：
 * 	intege 整数   			intege1 正整数
	intege2 负整数			num: 数字
	num1 正数（正整数 + 0）	num2 负数（负整数 + 0）
	decmal 浮点数			decmal1 正浮点数
	decmal2 负浮点数			decmal3 浮点数
	decmal4 非负浮点数（正浮点数 + 0）	decmal5 非正浮点数（负浮点数 + 0）

	email 邮件				color 颜色
	url url					chinese 仅中文
	ascii 仅ACSII字符		zipcode 邮编
	mobile 手机				ip4 ip地址
	notempty 非空			picture 图片
	rar 压缩文件				date 日期
	qq QQ号码				tel 电话号码的函数(包括验证国内区号,国际区号,分机号)
	username 用来用户注册。匹配由数字、26个英文字母或者下划线组成的字符串	letter 字母
	letter_u 大写字母		letter_l 小写字母	idcard 身份证
 * @param regexText    正则表达式或是枚举的名称
 * @return
 */
function validateRegex(isEmpty,idStr,onShow,onFocus,onCorrect,onError,isEnum,regexText){
	
	if (isEmpty==false){
		$(idStr).after(redTip);
	}
	if (isEnum==true){
		$(idStr).formValidator({empty:isEmpty,onShow:onShow,onFocus:onFocus,onCorrect:onCorrect}).regexValidator({regExp:regexText,dataType:"enum",onError:onError});;
	}else{
		$(idStr).formValidator({empty:isEmpty,onShow:onShow,onFocus:onFocus,onCorrect:onCorrect}).regexValidator({regExp:regexText,onError:onError});;
	}
