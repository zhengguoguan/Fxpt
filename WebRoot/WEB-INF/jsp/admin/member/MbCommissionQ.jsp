	...
	function delConfirm(id){
		$.layer({
		  ...
		  dialog : {
		      ...
		      btn : ['是','否'],
		      yes : function(){
		          location.href='${ctx}/admin/mbCommission/delete/' + id + '.html';
		      },
		      no : function(index){
		         layer.close(index);
		      }
...
<div class="content_box">
  <div class="btn_box">
  		...
  		<input id="add_bt" type="button" value="添加" class="initial" onclick="javascript:location.href='${ctx}/admin/mbCommission/add/new.html'"/>
     
  </div>
   <div class="list_info">
  	<form id="search_form" action="${ctx}/admin/mbCommission/list/1.html" method="post">
  	  </form>
    <h2>分销提成设置</h2>
  ...
  <table width="98%" border="0" cellpadding="0" cellspacing="1">
      ...
      <c:forEach items="${list}" var="mb" varStatus="sta">
	      <tr ondblclick="javascript:location.href='${ctx}/admin/mbCommission/add/new.html?id=${mb.id}'">
	        <td>${sta.index + 1}</td>
	      	<td>${mb.mbtype}</td>
	      	<td>${mb.commission}
	      	</td>
	        <td>
	          	<c:if test="${mb_updt == true}">
		          	<div class="btn_icon">
		          	 <input type="image" src="${ctx}/theme/default/images/edit_icon.png" title="修改" onclick="javascript:location.href='${ctx}/admin/mbCommission/add/new.html?id=${mb.id}'"/>
		          	</div>
	          	</c:if>
